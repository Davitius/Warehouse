      ...
      <div class="collapse navbar-collapse" id="navbarsExample05">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            ...
            <li class="nav-item">
            <a class="nav-link active" href="orders.php">გაყიდული საქონელი</a>
          </li>
            <li class="divider"></li>
            <li class="nav-item">
  ...
  <main class="px-3">
    <div class="container cv col-md-12">
        <div class="row centered">
          ...
          <label class="white text">გაყიდული პროდუქტი</label>
...
<?php
     ...
     if(isset($_GET['search'])){
        ...
        $sql = "SELECT * FROM wareh_orders WHERE dro LIKE '%$searchKey%'";
     }else
     $sql = "SELECT * FROM wareh_orders ORDER BY id DESC";
     $result = $conn->query($sql);
?>
   ...
   <!-- ძებნა თარიღით  -->
    <form action="" method="GET">
        <div class="btn-group">
          <input type="text" name="search" class='form-control' placeholder="თარიღი" value=<?php echo @$_GET['search']; ?> >
          <input class="btn btn-primary" type="submit" value="ძებნა"/>
         <a class="btn btn-primary" href="orders.php">განახლება</a>
        </div>
   </form>

<!-- გაყიდული საქონლის სია  -->
...
<?php while( $roww = $result->fetch_object() ): ?>
        <tr>
            <td><?php echo $roww->id ?></td>
            <td><?php echo $roww->dro ?></td>
            <td><?php echo $roww->product ?></td>
            <td><?php echo $roww->allprice ?></td>
            <td><?php echo $roww->seller ?></td>
        </tr>
<?php endwhile; ?>
                  </table>
