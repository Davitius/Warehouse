...
ob_start();
$user_id = $_GET['id'];
$user = mysqli_query($connect, "SELECT * FROM wareh_users WHERE id = '$user_id'");
 ...
 <div class="cover-container-fluid d-flex w-100 h-100 p-3 mx-auto flex-column">
  ...
  <main class="px-3">
    ...
    <div class="row centered">
                <h3 class="white text" >თანამშრომლის მონაცემები</h3>
        <!-- //უზერის ფოტო.  -->
<div class="col-md-6" >
    <br><br>
...
/* თანამშრომლის მონაცემის განახლება */
...
$userimg = addslashes(file_get_contents($_FILES['userimg']['tmp_name']));
    ...
    $query = "UPDATE `wareh_users` SET `user`='".$user."', `name`='".$name."', `position`='".$position."', `phone`='".$phone."', `status`='".$status."', `userimg`='".$userimg."', `pass`='".$pass."' WHERE `id` = $id";
    $query_run = mysqli_query($connect, $query) or die(mysqli_error());

                                               }
    else{
        $query = "UPDATE `wareh_users` SET `user`='".$user."', `name`='".$name."', `position`='".$position."', `phone`='".$phone."', `status`='".$status."', `pass`='".$pass."' WHERE `id` = $id";
    $query_run = mysqli_query($connect, $query) or die(mysqli_error());
         }
    ...
    {
        ob_end_clean();
        header('Location: ../php/errors/userupdateconfirm.php');
        exit();
    }
    else
    {
...
if(isset($_POST['userdelete'])){
        $id = $_GET['id'];
        $querry = "DELETE FROM `wareh_users` WHERE `id` = $id";
            $querry_run = mysqli_query($connect, $querry) or die(mysqli_error());

if($querry_run)
    {
        ob_end_clean();
        header('Location: ../php/errors/userdeleteconfirm.php');
        exit();
    }
